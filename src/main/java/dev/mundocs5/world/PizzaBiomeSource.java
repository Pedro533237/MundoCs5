...
public class PizzaBiomeSource extends BiomeSource {
    ...
    public PizzaBiomeSource(MultiNoiseBiomeSource vanillaBiomeSource, long layoutSeed, LandConfig landConfig, OceanConfig oceanConfig) {
        ...
        this.slices = List.of(
                new BiomeSlice(0.0, 0.125, landConfig.eastBiomes(), Climate.TEMPERATE),
                new BiomeSlice(0.125, 0.25, landConfig.southeastBiomes(), Climate.WARM),
                new BiomeSlice(0.25, 0.375, landConfig.southBiomes(), Climate.HOT),
                new BiomeSlice(0.375, 0.5, landConfig.southwestBiomes(), Climate.TEMPERATE),
                new BiomeSlice(0.5, 0.625, landConfig.westBiomes(), Climate.COOL),
                new BiomeSlice(0.625, 0.75, landConfig.northwestBiomes(), Climate.TEMPERATE),
                new BiomeSlice(0.75, 1.0, landConfig.northBiomes(), Climate.COLD)
        );
    ...
    public RegistryEntry<Biome> getBiome(int x, int y, int z, MultiNoiseUtil.MultiNoiseSampler noise) {
        ...
        if (baseDistance >= landConfig.outerWorldStart() + 384.0) {
        ...
        if (warpedDistance < landConfig.centerRadius()) {
            return landConfig.centerBiome();
        }

        if (warpedDistance >= landConfig.outerWorldStart()) {
            return vanillaBiomeSource.getBiome(x, y, z, noise);
        }
        ...
        BiomeSlice secondarySlice = warpedDistance < landConfig.oceanStart() ? findNeighborSlice(normalizedAngle, primarySlice) : primarySlice;
        ...
        boolean nearCoast = warpedDistance >= landConfig.oceanStart() - beachWidth
                && warpedDistance < landConfig.oceanStart() + beachWidth * 0.35;

        if (warpedDistance < landConfig.oceanStart()) {
        ...
        double ringBlend = MathHelper.clamp(
                (warpedDistance - landConfig.oceanStart()) / Math.max(1.0, landConfig.outerWorldStart() - landConfig.oceanStart()),
                0.0,
                1.0
        );
    ...
    private RegistryEntry<Biome> pickLandBiome(BiomeSlice primarySlice, BiomeSlice secondarySlice, double normalizedAngle, double distance, double x, double z) {
        List<RegistryEntry<Biome>> primaryGroup = ensureNotEmpty(primarySlice.biomes(), landConfig.centerBiome());
        List<RegistryEntry<Biome>> secondaryGroup = ensureNotEmpty(secondarySlice.biomes(), primaryGroup.get(0));

        int primaryIndex = Math.floorMod(
                (int) Math.floor((distance + OrganicNoise.sample(layoutSeed ^ 0xBEEFL, x, z, 300.0, 2) * 240.0) / 340.0),
                primaryGroup.size()
        );
        ...
        int secondaryIndex = Math.floorMod(
                (int) Math.floor((distance + OrganicNoise.sample(layoutSeed ^ 0xD00DL, x, z, 320.0, 2) * 220.0) / 360.0),
                secondaryGroup.size()
        );
    ...
    private RegistryEntry<Biome> pickOceanBiome(double normalizedAngle, double warpedDistance, double x, double z) {
        ...
        double depthBlend = MathHelper.clamp(
                (warpedDistance - landConfig.oceanStart()) / Math.max(1.0, landConfig.outerWorldStart() - landConfig.oceanStart()),
                0.0,
                1.0
        );
        ...
        if (warmness > 0.28) {
            return depthBlend > 0.46 ? oceanConfig.lukewarmOceanBiome() : oceanConfig.warmOceanBiome();
        }

        if (warmness < -0.3) {
            return depthBlend > deepThreshold ? oceanConfig.deepColdOceanBiome() : oceanConfig.coldOceanBiome();
        }
    ...
    private RegistryEntry<Biome> beachFor(Climate climate) {
        return climate == Climate.COLD || climate == Climate.COOL ? oceanConfig.snowyBeachBiome() : oceanConfig.beachBiome();
    }
    ...
    private List<RegistryEntry<Biome>> collectBiomes() {
        ...
        collected.addAll(landConfig.eastBiomes());
        collected.addAll(landConfig.southeastBiomes());
        collected.addAll(landConfig.southBiomes());
        collected.addAll(landConfig.southwestBiomes());
        collected.addAll(landConfig.westBiomes());
        collected.addAll(landConfig.northwestBiomes());
        collected.addAll(landConfig.northBiomes());
        collected.add(oceanConfig.temperateOceanBiome());
        collected.add(oceanConfig.coldOceanBiome());
        collected.add(oceanConfig.deepColdOceanBiome());
    ...
    public record LandConfig(
        ...
        public static final Codec<LandConfig> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                ...
                BIOME_CODEC.listOf().fieldOf("southeast_biomes").forGetter(LandConfig::southeastBiomes),
                BIOME_CODEC.listOf().fieldOf("south_biomes").forGetter(LandConfig::southBiomes),
                BIOME_CODEC.listOf().fieldOf("southwest_biomes").forGetter(LandConfig::southwestBiomes),
                BIOME_CODEC.listOf().fieldOf("west_biomes").forGetter(LandConfig::westBiomes),
                BIOME_CODEC.listOf().fieldOf("northwest_biomes").forGetter(LandConfig::northwestBiomes),
                BIOME_CODEC.listOf().fieldOf("north_biomes").forGetter(LandConfig::northBiomes),
                Codec.intRange(1, Integer.MAX_VALUE).optionalFieldOf("pizza_radius", 3000).forGetter(LandConfig::pizzaRadius),
                Codec.intRange(1, Integer.MAX_VALUE).optionalFieldOf("ocean_start", 3000).forGetter(LandConfig::oceanStart),
                Codec.intRange(1, Integer.MAX_VALUE).optionalFieldOf("outer_world_start", 4048).forGetter(LandConfig::outerWorldStart)
        ).apply(instance, LandConfig::new));

        public LandConfig {
            eastBiomes = List.copyOf(eastBiomes);
            southeastBiomes = List.copyOf(southeastBiomes);
            southBiomes = List.copyOf(southBiomes);
            southwestBiomes = List.copyOf(southwestBiomes);
            westBiomes = List.copyOf(westBiomes);
            northwestBiomes = List.copyOf(northwestBiomes);
            northBiomes = List.copyOf(northBiomes);
        }
    }
